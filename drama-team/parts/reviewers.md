...
| 12 | **大纲承诺兑现** | 5 | **（v3.9 ⭐ 新增）** 本集是否兑现了 outline.md 分集梗概中的"大纲承诺"项？检查：①承诺的揭示/转折/事件是否发生 ②承诺的角色出场是否出现 ③承诺的伏笔操作（埋/回收）是否执行。未兑现→-5分/项 |
| | **合计** | **105** | → 归一化到100分（得分×100/105） |
...
| 4 | **时空切换** | 10 | 🔗 闪回/场景切换是否有衔接三件套：① 动作承接（角色物理反应）② 视觉锚点（MATCH CUT 同一道具）③ 音效联动。情感转折是否有前置铺垫镜头（1秒微表情/动作） |
| 5 | **AI指令颗粒度** | 15 | 🤖 每个镜头是否有 AI 可执行的指令：① 特效参数明确（位置/颜色/透明度）② AI约束词（面部不扭曲/液体不穿模/肢体不穿插）③ 光影描述具体 |
| 6 | **情绪递进** | 10 | 🌊 **动态评估**：这集情绪终点是什么？镜头是否支撑弧线？不强制"平静→爆发→回落"，而是检查：① 情绪变化是否被镜头语言"看见" ② 角色弧光转折是否有运镜/光影对比（如恐惧=特写+摇晃 vs 决心=中景+稳定） |
| 7 | **跨集视觉承接** | 10 | 🔗 **（v3.9 ⭐ 新增）** 本集前3镜是否与 `visual_continuity.md` 中上集结尾快照视觉连续？检查：① 角色画面位置/朝向是否合理衔接（不能无理由左右跳变）② 色调是否延续或有合理过渡（不能无理由暖→冷跳变）③ 场景物理状态是否延续（上集门碎了下集不能完好）④ 服装是否延续（上集换装下集不能穿回旧装）⑤ 光源方向是否一致 |
| | **合计** | **100** | |
